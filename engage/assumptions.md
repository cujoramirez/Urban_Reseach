# Our Assumptions (UNVERIFIED)

Everything on this page is something we currently **believe** about walking in the Sudirman
area but have **not yet checked** against reality. These are *hypotheses to test in the
field*, not facts.

> ⚠️ **Keep assumptions OUT of the guiding questions.** An assumption written into a question
> turns it into a leading question and biases the whole study. Assumptions live *here*. The
> questions in [`guiding-questions.md`](guiding-questions.md) stay neutral.

## How to use this file

- Populate it from our Miro board — pull out every belief we are carrying into the field.
- Label each one **UNVERIFIED** until fieldwork confirms or refutes it.
- After fieldwork, update the **Status** column. Do **not** delete a refuted assumption —
  a refuted assumption is a finding.

## Assumptions table

| # | Assumption | Status | How we'll test it |
|---|------------|--------|-------------------|
| 1 | Sidewalks are frequently blocked by parked motorbikes / street vendors. | UNVERIFIED | Audit sheet (obstruction parameter) + photos |
| 2 | The back-streets one block off Sudirman have little or no usable sidewalk. | UNVERIFIED | Audit sheet (sidewalk present / effective width) |
| 3 | People step into the road because the sidewalk is obstructed or missing. | UNVERIFIED | Observation counts + intercept questions |
| 4 | Heat / lack of shade makes walking unpleasant during the day. | UNVERIFIED | Audit (shade parameter) + intercept on comfort |
| _…_ | _(add the rest from Miro)_ | UNVERIFIED | |

---

**Reminder:** the point of fieldwork is to find out whether these are true — including the
possibility that they are wrong. Let the data decide.
