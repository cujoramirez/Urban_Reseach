...
|------------|---------------------|--------|-----|
| Effective sidewalk **width** | **≥ 1,85 m** minimum; formula **W = V/35 + N** (floor 1,85 m) | S13 Bina Marga 2023 | ✓ |
| Sidewalk **dimension vs flow** | CBD arterial: max **80 ped/min**, effective **2,75–3,75 m** (total 5–6 m); CBD collector: **60 ped/min**, effective **2–2,75 m** (Table 3) | S13 | ✓ |
...
| **Climate-coping** (umbrella/parasol use, shade-hugging, rain-sheltering, ojol surge in rain) | no fixed benchmark — count as the **behavioural proxy** for thermal/rain discomfort | observation | — |
...
| Umbrella habit / weather coping | self-reported frequency (selalu→tidak) vs observed umbrella tally (Section F); rain mode-shift to ojol | observation + interview | — |
...
| Pedestrian safety — was it a driver? | **10.428** ped crash victims 2023, crossing = top cause; 1 ped death/6 days (2014, DKI) | S14; S6 | ✓ |
...
- **Closed since round 4:** Bina Marga Table 3 + width formula (W = V/35 + N) extracted (S13);
  pedestrian victims **10.428 (2023)** verified (S14/CAP-11); TOD distance **400/800 m** verified
  (CAP-12).
- **⚠ Still verify:** the 54,84% careless-crossing share (direction confirmed, exact figure not);
  walking-speed review (MDPI/ScienceDirect both blocked here); thermal THI (Karyono primary);
  MRT/TJ 2024 ridership official pages (JS-rendered, fetch empty); HCM PLOS thresholds.
- **No corridor-specific** number exists for: the back-street gradient (G1) or Sudirman-area
  harassment (G8). The **fieldwork generates** these, and now has benchmarks to measure against.
