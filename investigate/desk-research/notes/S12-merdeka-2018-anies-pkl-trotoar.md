# S12 · Anies: Trotoar Sudirman–Thamrin bukan tempat jualan PKL (Merdeka, 2018)

## Full citation
Merdeka.com. (2018, 6 March). *Anies: Trotoar Sudirman-Thamrin bukan tempat jualan PKL, buat
pejalan kaki!* (Statement by Gov. Anies Baswedan.)

- **URL:** https://www.merdeka.com/jakarta/anies-trotoar-sudirman-thamrin-bukan-tempat-jualan-pkl-buat-pejalan-kaki.html
- **Capture:** `sources/web-captures.md` → CAP-9
- **Source type:** News (government statement).
- **Accessed:** 2026-06-16

## Scope
**CORRIDOR.** Policy specific to the Sudirman–Thamrin sidewalk, with explicit spillover to the
adjacent **side streets** (the gradient).

## POLICY / QUALITATIVE (exact passages)
- "**Trotoar sepanjang Sudirman-Thamrin bukan tempat berjualan PKL, di sini tempat untuk pejalan
  kaki.**" (Anies, 6 Mar 2018)
- Plan: **relocate PKL to side streets behind the office buildings**; provide kiosks only for
  newspapers/transit cards (not food); target before Asian Games 2018.

## Why it matters for fieldwork
- Documents the **policy mechanism that cleared the main trotoar** of vendors and **pushed
  informal trade toward the back-streets**, a plausible *driver* of the wide-main-vs-busy-back
  gradient (G1) the team will study on the ground.
- Identifies **PKL (street vendors) as a key corridor actor** whose livelihood interest competes
  with pedestrian space, a group to interview, and a concrete "obstacle" theme for the
  expert/policy interview.

## SENTIMENT / INTENT
- Government framing: sidewalk = pedestrians-first; vendors to be accommodated elsewhere. (Label
  as stated policy intent.) Note from search context (unverified): vendors reportedly return in
  practice, **a tension to test in the field, not assert.**

## Corridor vs. general
- Corridor-specific policy; the relocation-to-side-streets detail is the corridor→back-street link.

## Flags
- 2018 statement; enforcement reality has fluctuated since (see S10 city-wide context, and CFD
  PKL bans/Satpol PP episodes noted as unverified leads). Use as the policy anchor, verify
  current state in the field.
